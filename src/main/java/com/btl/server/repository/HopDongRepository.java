...
public interface HopDongRepository extends JpaRepository<HopDong, Long> {
    ...
    // Lệnh Bulk Update tự động Từ chối hàng loạt (TU_CHOI) tất cả các yêu cầu thuê phòng đang chờ duyệt khác của cùng một căn phòng trọ đó ngay sau khi Chủ trọ quyết định Duyệt cho một khách thuê cụ thể. Sử dụng `@Modifying` để báo cho Hibernate đồng bộ lại Cache cấp 1.

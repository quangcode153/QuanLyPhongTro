...
// REST Controller điều phối các API quản lý Hợp Đồng thuê phòng (`/api/hop-dong`). Cung cấp các thao tác: Khách gửi yêu cầu thuê phòng, Chủ trọ phê duyệt/từ chối/gia hạn/thanh lý hợp đồng, và khách thuê chủ động xin hủy hợp đồng.
@RestController
...
public class HopDongController {
    ...
    // API Khách thuê (vai trò ROLE_USER) tạo yêu cầu thuê phòng trọ mới. Mặc định tạo hợp đồng ở trạng thái chờ duyệt (CHO_DUYET).
    @PostMapping
    ...
    // API Chủ trọ xem toàn bộ các hợp đồng thuộc phạm vi quản lý của mình. Bảo mật: Chặn không cho chủ trọ này cố tình xem hợp đồng của chủ trọ khác.
    ...
    // API Khách thuê lấy danh sách hợp đồng của riêng họ. Hỗ trợ bộ lọc theo trạng thái hợp đồng (ví dụ: chỉ lấy các hợp đồng đã duyệt DA_DUYET).
    ...
    // API Chủ trọ (hoặc Admin) cập nhật trạng thái hợp đồng thuê phòng. Quy tắc nghiệp vụ (Business Rules): - Duyệt hợp đồng (DA_DUYET): Chuyển phòng sang DA_THUE, bulk update tự động từ chối các yêu cầu thuê khác. - Từ chối hợp đồng (TU_CHOI): Giữ nguyên trạng thái phòng trống.
    ...
    // API Chủ trọ thanh lý hợp đồng thuê (DA_THANH_LY). Nghiệp vụ: Chốt trả lại tiền cọc (sau khi trừ các hư hỏng tài sản nếu có), đưa phòng trọ về trạng thái trống.
    ...
    // API Khách thuê gửi yêu cầu đơn phương hủy hợp đồng thuê trước thời hạn (HUY). Nghiệp vụ: Khách chấp nhận chịu mất cọc hoặc bồi thường theo đúng thỏa thuận ban đầu.

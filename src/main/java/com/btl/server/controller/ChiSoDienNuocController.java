...
/**
 * REST Controller điều phối các API ghi nhận chỉ số Điện và Nước hàng tháng (`/api/dien-nuoc`).
 * Hỗ trợ chốt số tháng mới, cập nhật chỉ số khi có sai sót và tra cứu chỉ số chu kỳ.
 */
@RestController
...
public class ChiSoDienNuocController {
    ...
    /**
     * API Chủ trọ thực hiện chốt số điện nước đầu/cuối tháng của phòng trọ.
     * Hệ thống tự động tính toán tổng tiêu thụ Delta và xuất hóa đơn dịch vụ tương ứng.
     * 
     * @param chiSo Thực thể ChiSoDienNuoc chứa số điện cũ/mới, số nước cũ/mới
     */
    @PostMapping("/chot-so")
    ...
    /**
     * API Chủ trọ cập nhật sửa đổi lại chỉ số điện nước chu kỳ do ghi nhận sai sót.
     * Hệ thống tự động tính toán lại dư nợ và sửa trực tiếp trên Hóa đơn tương ứng chu kỳ đó.
     * 
     * @param hoaDonId ID của hóa đơn dịch vụ cần sửa chỉ số đi kèm
     * @param chiSoMoi Thông tin chỉ số mới cần lưu
     */

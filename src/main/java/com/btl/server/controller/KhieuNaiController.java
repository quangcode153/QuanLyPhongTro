...
/**
 * REST Controller tiếp nhận toàn bộ các API xử lý sự cố khiếu nại (`/api/khieu-nai`).
 * Khách thuê có thể gửi phản ánh lỗi phòng ốc, dịch vụ, và Admin thực hiện đánh dấu xử lý hoàn tất.
 */
@RestController
...
public class KhieuNaiController {
    ...
    /**
     * API Thành viên (Khách thuê hoặc Chủ trọ) gửi khiếu nại phản hồi lỗi dịch vụ hoặc cơ sở vật chất.
     * Mặc định tạo sự cố ở trạng thái chờ xử lý (CHO_XU_LY).
     * 
     * @param request DTO chứa tiêu đề và nội dung khiếu nại
     */
    @PostMapping
     ...
     * @param id ID của khiếu nại cần đánh dấu
     */
    @PutMapping("/{id}/xu-ly")
    @PreAuthorize("hasRole('ADMIN')")
